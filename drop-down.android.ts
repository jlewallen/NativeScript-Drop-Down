...
export class DropDown extends DropDownBase {
  ...
  public createNativeView() {
    ...
    const itemSelectedListener = new DropDownItemSelectedListener(
      new WeakRef(this)
    );
  ...
  public [fontInternalProperty.setNative](
    value: Font | android.graphics.Typeface
  ) {
  ...
  public [placeholderColorProperty.setNative](value: Color | number) {
    this._propagateStylePropertyToRealizedViews(
      "placeholderColor",
      value,
      true
    );
  ...
  private _propagateStylePropertyToRealizedViews(
    property: string,
    value: any,
    isIncludeHintIn = true
  ) {
  ...
  private _updateSelectedIndexOnItemsPropertyChanged(
    newItems: any[] | ItemsSource
  ) {
    let newItemsCount = 0;
