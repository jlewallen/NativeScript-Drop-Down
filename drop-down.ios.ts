...
export class DropDown extends DropDownBase {
  ...
  public initNativeView() {
    ...
    this._dropDownDelegate = DropDownListPickerDelegateImpl.initWithOwner(
      new WeakRef(this)
    );
    this._dropDownDataSource = DropDownListDataSource.initWithOwner(
      new WeakRef(this)
    );
    ...
    this._toolbar = UIToolbar.alloc().initWithFrame(
      CGRectMake(0, 0, applicationFrame.size.width, TOOLBAR_HEIGHT)
    );
  ...
  public [fontInternalProperty.setNative](value: Font | UIFont) {
    const font =
      value instanceof Font ? value.getUIFont(this.nativeView.font) : value;
...
class DropDownListDataSource
  ...
  public pickerViewNumberOfRowsInComponent(
...
class DropDownListPickerDelegateImpl
  extends NSObject
  ...
  public pickerViewViewForRowForComponentReusingView(
    ...
    if (owner.nativeView.itemsPadding !== itemsPaddingProperty.defaultValue) {
      const itemsPadding = owner.nativeView.itemsPadding
        .split(/[ ,]+/)
        .map((s) => Length.parse(s));
...
class TNSDropDownLabel extends TNSLabel {
  public static initWithOwner(owner: WeakRef<DropDown>): TNSDropDownLabel {
    ...
    label.addGestureRecognizer(
      UITapGestureRecognizer.alloc().initWithTargetAction(label, "tap")
    );
  ...
  @ObjCMethod()
  public tap(
    @ObjCParam(UITapGestureRecognizer) sender: UITapGestureRecognizer
  ) {
...
function _setTextAttributes(nativeView: TNSLabel, style: Style) {
  ...
  if (style.letterSpacing !== 0) {
    attributes.set(
      NSKernAttributeName,
      style.letterSpacing * nativeView.font.pointSize
    );
  }
  ...
  const text: string = types.isNullOrUndefined(nativeView.text)
    ? ""
  ...
  if (attributes.size > 0) {
    const result = NSMutableAttributedString.alloc().initWithString(
      sourceString
    );
    result.setAttributesRange(attributes as any, {
      location: 0,
